...
export class SubCategoryController extends BaseController {
  ...
  async createSubCat(req, res, next) {
    try {
      ...
      const info = req.body;
      const data = await subCategoryService.createSubCat(
        userToken,
        catId,
        info
      );
  ...
  async updateSubCat(req, res, next) {
    try {
      ...
      const data = await subCategoryService.updateSubCat(
        token,
        subCatId,
        newData
      );
  ...
  async removeSubCategory(req, res, next) {
    try {
      ...
      if (data == 403) {
        res
          .status(403)
          .send("NO USER FOUND OR YOU DON'T HAVE ACCESS TO DO THIS");
      } else {
        res.status(200).send(data);
      }
    } catch (error) {
