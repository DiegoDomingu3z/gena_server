...
class EmailService {
  async sendUserCredentials(data, token, pass) {
    try {
      ...
      if (data.email == "" || data.email == undefined) {
        ...
        const department = await dbContext.Department.findById(
          data.departmentId
        );
        ...
        await transporter.sendMail({
          ...
          html: `
                    ...
                    <br> <b> Team-Lead: </b> ${
                      lead == null ? "N/A" : lead.firstName + lead.lastName
                    } 
                    <br> <b> Group-Lead: </b>  ${
                      group == null ? "N/A" : group.firstName + group.lastName
                    } 
  ...
  async ticketingSystem(data, token) {
    try {
      const reportedUser = await dbContext.Account.findOne({
        accessToken: token,
      });
  ...
  async updateUserAccountEmail(data, pass) {
    try {
      ...
      let body = `
               ...
               <p><b>Group lead:</b> ${
                 data.groupLead == "" ? "N/A" : data.groupLead
               }</p>
               <p><b>Team lead:</b> ${
                 data.teamLead == "" ? "N/A" : data.teamLead
               }</p>
  ...
  async readyForPickupEmail(order) {
    try {
      ...
      for (let i = 0; i < order.labels.length; i++) {
        ...
        const labelDoc = await dbContext.Label.findOne({ _id: label.labelId });
        docNums.push(
          `<b>${labelDoc.docNum} </b> QTY(${label.qty}) ${
            label.serialRange ? `<br>Serial Range: ${label.serialRange}` : ""
          } <br>------------<br><br>`
        );
      }
      docNums.toString();
