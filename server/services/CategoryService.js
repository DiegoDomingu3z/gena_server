    ...
    try {
      ...
      if (!user) {
        ...
        if (!cat) {
          ...
          const updatedDoc = await dbContext.Category.findByIdAndUpdate(
            id,
            {
              path: newPath,
              updatedOn: todayDate,
          ...
          const updatedLabels = await dbContext.Label.updateMany(
            { categoryId: id },
            { $set: { categoryName: data.name } }
          );
          const subCats = await dbContext.SubCategory.updateMany(
            { categoryId: id },
            { path: newPath, bulkPath: bulkPath }
          );
          return updatedDoc;
        }
      }
