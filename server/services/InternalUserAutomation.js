...
class InternalUserAutomation {
  ...
  async gatherDepartments() {
    try {
      ...
      const updatedDepts = await dbContext.Department.bulkWrite(
        updateOperations
      );
      return updatedDepts;
    } catch (error) {
      ...
      do {
        ...
        try {
          do {
            ...
            var groupLeads = groupLeadsCounter.filter(
              (e) =>
                e.employee_position == "Group Lead" ||
                e.employee_position == "ITD Vice President"
            );
            const currentGroupLeads = await dbContext.Account.find({
            ...
            for (let i = 0; i < groupLeads.length; i++) {
              ...
              const dept = await dbContext.Department.findOne({
                name: GL.employee_department,
              });
              if (GL.firstName == "") {
                continue;
              } else if (
                !currentGLEmails.includes(GL.employee_email) &&
                !emails.includes(GL.employee_email)
              ) {
                // MAKES SURE GENA DB DOES NOT INCLUDE THAT USER ALREADY
                const year = today.getFullYear();
                let password = await middleware.encryptPassword(
                  `${GL.lastName}${year}`
                );
                const data = {
                ...
                const updatedEmployee = await dbContext.Account.updateOne(
                  filter,
                  update
                );
      ...
      do {
        ...
        try {
          do {
            ...
            var teamLeads = teamLeadsCounter.filter(
              (e) => e.employee_position == "Team Lead"
            );
            const currentTeamLeads = await dbContext.Account.find({
              privileges: "group-lead",
            });
            ...
            for (let i = 0; i < teamLeads.length; i++) {
              ...
              const dept = await dbContext.Department.findOne({
                name: TL.employee_department,
              });
              groupLead = await dbContext.Account.findOne({
                department: dept.name,
                privileges: "group-lead",
              });
              if (dept.name == "night-shift") {
                groupLead = await dbContext.Account.findOne({
                  email: "ClayP@inventive-group.com",
                });
              } else if (dept.name == "sales") {
                groupLead = await dbContext.Account.findOne({
                  email: "BenC@inventive-group.com",
                });
              }
              if (TL.firstName == "") {
                continue;
              } else if (
                !currentTLEmails.includes(TL.employee_email) &&
                !emails.includes(TL.employee_email)
              ) {
                const year = today.getFullYear();
                let password = await middleware.encryptPassword(
                  `${TL.lastName}${year}`
      ...
      do {
        try {
          ...
          do {
            ...
            var teamMembers = teamMembersCounter.filter(
              (e) => e.employee_position == "Team Member"
            );
            for (let i = 0; i < teamMembers.length; i++) {
              const TM = teamMembers[i];
              const dept = await dbContext.Department.findOne({
                name: TM.employee_department,
              });
              groupLead = await dbContext.Account.findOne({
                department: dept.name,
                privileges: "group-lead",
              });
              const teamLeader = await dbContext.Account.findOne({
                department: dept.name,
                privileges: "team-lead",
              });
              if (dept.name == "night-shift") {
              ...
              if (TM.firstName == "") {
                ...
                const userExists = await dbContext.Account.findOne({
                ...
                if (!userExists) {
                  ...
                  const createdTeamMember = await dbContext.Account.create(
                    data
                  );
                  ...
                  const updatedUser = await dbContext.Account.updateOne(
                    filter,
                    update
                  );
